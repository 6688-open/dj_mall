	...
	<%--<form id = "fm">
		...
		分类上级	<select name="parentCode">
				...
				<p>
					...
					<input id="dictionaryName" name="dictionaryName" type="text" placeholder="分类名称" maxlength="6"/>
				</p>
				<p>
					<label for="code">分类code：</label>
					<input id="code" name="code" type="text" placeholder="分类code" maxlength="10">
				</p>
		...
		<form id = "fmt">
			...
			<input type="hidden" value="reset" onclick="search(page(1))" />
		</form>

	<h2 align="center">字典数据展示</h2>
...
<script type="text/javascript">
	...
	function search() {
		$.get(
				"<%=request.getContextPath()%>/dictionary/list",
				$("#fmt").serialize(),
				function (data) {
					var html = "";
					for (var i = 0; i < data.data.list.length; i++) {
						var dic = data.data.list[i];

							html += "<tr>";
							html += "<td>"+dic.code+"</td>";
							html += "<td>"+dic.dictionaryName+"</td>";
							html += "<td>"+dic.parentCode+"</td>";
							html += "<td><input type='button' value='修改' onclick='up("+dic.id+")'/></td>";
							html += "</tr>";
	...
	function page(pageNo,totalPage) {
		...
		$(function () {
			...
			$("#fm").validate({
				...
				rules:{
					...
					code: {
						...
						remote:{
							url:"<%=request.getContextPath()%>/dictionary/uniq",
							type:"post",
							data:{userName:function(){return $("#code").val()}}
						}
					},
				...
				messages:{
					...
					code: {
						required: "请输入分类code",
						minlength: "分类code至少由2个字母/数字/汉字组成",
						remote:"分类code已存在,请重新添加"
					},
				}
			})
		})
		...
		$.validator.setDefaults({
			submitHandler: function(form) {
				...
				if(result){
					...
					$.post(
							"<%=request.getContextPath()%>/dictionary/insert",
							$("#fm").serialize(),
							function (data) {
								if(data.code == 200){
									layer.msg(data.msg, {icon: 6},function () {
										window.location.href = "<%=request.getContextPath()%>/dictionary/toList"
		...
		function up(id) {
			layer.open({
				...
				shade: 0.8,
				area: ['280px', '50%'],
				content:'<%=request.getContextPath()%>/dictionary/toUpdate/'+id ,
