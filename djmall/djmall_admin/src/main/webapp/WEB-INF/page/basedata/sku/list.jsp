    ...
    <div align="center">
        <form id="fm">
            <p>
                ...
                <input id="attrName" name="attrName" type="text" placeholder="属性名" maxlength="6"/>
            </p>

            <input type='submit' value='新增商品属性' onclick='add()'/>
        </form>
    </div>
...
<script type="text/javascript">
    ...
    function search() {
        $.get(
            "<%=request.getContextPath()%>/sku/list",
            $("#fmt").serialize(),
            function (data) {
                var html = "";
                for (var i = 0; i < data.data.productAttrList.length; i++) {
                    var attr = data.data.productAttrList[i];

                    html += "<tr>";
                    html += "<td>"+attr.id+"</td>";
                    html += "<td>"+attr.attrName+"</td>";
                    html += "<td>"+attr.attrValueShow+"</td>";
                    html += "<td><input type='button' value='关联属性值' onclick='up("+attr.id+",\""+attr.attrName+"\")'/></td>";
                    html += "</tr>";
    ...
    function up(id, attrName) {
        layer.open({
            ...
            shade: 0.8,
            area: ['480px', '100%'],
            content:'<%=request.getContextPath()%>/sku/relationAttrValue/'+id+"/"+attrName,
