...
<center>
    ...
    <form id="fm">
        <p>
            <input id="logId" name="logId" type="hidden" value="${freight.logId}" />
            <input id="id" name="id" type="hidden" value="${freight.id}"/>
            <input type="hidden" name="_method" value="PUT">
            <label for="freight">运费：</label>
            <input id="freight" name="freight" onblur="freightClick()"  type="text" value="${freight.freight}" maxlength="6"/>
        </p>
        <p>
...
<script type="text/javascript">
    ...
    $.validator.setDefaults({
        submitHandler: function(form) {
            ...
            $.post(
                "<%=request.getContextPath()%>/log/update",
                $("#fm").serialize(),
                function(data){
                    if(data.code == 200){
                        layer.msg(data.msg, {
                            ...
                            parent.location.href = "<%=request.getContextPath()%>/log/toList";
                        });
                    } else {
                        layer.close(index1);
