	...
	<h2 align="center">商品管理展示</h2>
		<div align="left" style="width:800px ; height:160px;"  >
			<form id = "fmt">
				...
				名称:<input type="text" name="productName"  placeholder="名称关键字" size="8px" maxlength="6"  /><br><br>
				分类:
				...
				<center><input type="file" name="file"/> 选择文件</center>
			</form>
		</div>

		<%--获取当前登陆人的 等级   仅管理员可见--%>
		<input type="hidden" value="${user1.lever}" id="lever" />
		<c:if test="${user1.lever == 2}">
			<input type="button" value="刷新索引" onclick="makeSolr()" />
		</c:if>
...
<script type="text/javascript">
	...
	function search() {
		$.post(
				...
				function (data) {
					...
					divhtml += "<input type='button' value='查看评论' onclick='common()'/>------";
					divhtml += "<input type='button' value='导入' onclick='importPro()'/>";
					divhtml += "<input type='button' value='下载模板' onclick='uploadTemp()'/>";
					$("#div").html(divhtml);
	...
	function page(pageNo,totalPage) {
		...
		function makeSolr() {
			...
			$.post(
					"<%=request.getContextPath()%>/product/makeSolrNew",
					$("#fmt").serialize()
					,function(data){
						if(data.code == 200){
							layer.msg('刷新索引成功', {
								...
								/*parent.location.href = "<%=request.getContextPath()%>/exp/toList";*/
							});
						} else {
							layer.close(index1);
  ...
  function common() {
	 ...
	 /* layer.open({
		  ...
		  shade: 0.8,
		  area: ['90%', '90%'],
		  content:'<%=request.getContextPath()%>/product/toCommon/'+id,
		  end: function(){              //修改完刷新页面
			  location.reload();
		  }
	  });*/
	  window.location.href = "<%=request.getContextPath()%>/product/toCommon?id="+id;
		...
		//下载模板
		function uploadTemp() {
			window.location.href = "<%=request.getContextPath()%>/product/uploadTemp";
		}
		...
		function importPro() {
			...
			$.ajax({
				url:'<%=request.getContextPath()%>/product/importPro', /*接口域名地址*/
				type:'post',
				data: formData,
				// 告诉jQuery不要去处理发送的数据
				processData : false,
